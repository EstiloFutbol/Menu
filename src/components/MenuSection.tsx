...
export default function MenuSection() {
  ...
  const renderSlotContents = (day: { date: string; label: string }, meal: (typeof mealTypes)[number], compact = false) => {
    ...
    return <div className="space-y-1.5">{planned.map((item) => { const completed = entry?.consumption_logs?.some((log) => log.menu_entry_recipe_id === item.id) ?? false; return <div key={item.id} className={`rounded-lg ${compact ? '' : 'bg-neutral-50 px-2 py-1.5'}`}><div className="flex items-center gap-1"><p className="min-w-0 flex-1 truncate text-xs font-medium text-neutral-700">{item.recipe?.name ?? 'Receta'}</p>{completed && <CheckCircle2 size={12} className="shrink-0 text-emerald-600" />}</div>{!compact && <div className="mt-1 flex items-center justify-between gap-2"><span className="text-[11px] text-neutral-400">{Number(item.servings).toLocaleString('es-ES')} r.</span>{!completed && <button onClick={(event) => { event.stopPropagation(); setCompletionTarget({ kind: 'recipe', plannedRecipeId: item.id, recipeId: item.recipe_id, recipeName: item.recipe?.name ?? 'Receta', servings: Number(item.servings) }) }} className="text-[11px] font-medium text-neutral-700 underline decoration-neutral-300 underline-offset-2">Completar</button>}</div>}</div> })}{products.map((item) => { const completed = entry?.consumption_logs?.some((log) => log.menu_entry_product_id === item.id) ?? false; return <div key={item.id} className={`rounded-lg ${compact ? '' : 'bg-neutral-50 px-2 py-1.5'}`}><div className="flex items-center gap-1"><p className="min-w-0 flex-1 truncate text-xs font-medium text-neutral-700">{item.ingredient?.name ?? 'Producto'}</p>{completed && <CheckCircle2 size={12} className="shrink-0 text-emerald-600" />}</div>{!compact && <div className="mt-1 flex items-center justify-between gap-2"><span className="text-[11px] text-neutral-400">{formatQuantity(Number(item.quantity))} {item.unit}</span>{!completed && <button onClick={(event) => { event.stopPropagation(); setCompletionTarget({ kind: 'product', entryId: entry!.id, productId: item.id, productName: item.ingredient?.name ?? 'Producto', quantity: Number(item.quantity), unit: item.unit }) }} className="text-[11px] font-medium text-neutral-700 underline decoration-neutral-300 underline-offset-2">Completar</button>}</div>}</div> })}</div>
  }
