...
function PantrySection() {
  ...
  const [view, setView] = useState<'stock' | 'products'>('stock')
  ...
  return <div className="space-y-5">
    <div className="grid grid-cols-2 rounded-2xl bg-neutral-100 p-1">
      <button type="button" onClick={() => setView('stock')} className={`rounded-xl px-4 py-2.5 text-sm font-medium ${view === 'stock' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500'}`}>Existencias</button>
      <button type="button" onClick={() => setView('products')} className={`rounded-xl px-4 py-2.5 text-sm font-medium ${view === 'products' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500'}`}>Productos</button>
    </div>

    {view === 'products' ? <ProductCatalogSection /> : <>
